...
module bluefin_spot::pool {
    ...
    public fun add_liquidity<CoinTypeA, CoinTypeB>(
        ...
        (0, 0, balance::zero<CoinTypeA>(), balance::zero<CoinTypeB>())
    }
    ...
    public fun remove_liquidity<CoinTypeA, CoinTypeB>(
        ...
        let token_a_amount = (balance::value(&pool.coin_a) as u128) * (liquidity as u128) / (pool.liquidity as u128);
        let token_b_amount = (balance::value(&pool.coin_b) as u128) * (liquidity as u128) / (pool.liquidity as u128);
        let token_a = balance::split(&mut pool.coin_a, (token_a_amount as u64));
        let token_b = balance::split(&mut pool.coin_b, (token_b_amount as u64));
        ((token_a_amount as u64), (token_b_amount as u64), token_a, token_b)
    }
    ...
    #[test_only]
    public fun create_pool_for_testing<A,B>(ctx: &mut TxContext) : Pool<A,B> {
        let pool = Pool<A,B> {
            id: object::new(ctx),
            ...
            //ticks_manager: TickManager::new(),
            //observations_manager: ObservationManager::new(),
            current_sqrt_price: 0,
            current_tick_index: i32::zero(),
            liquidity: 0,
            reward_infos: vector[],
            position_index: 0,
            icon_url: string::utf8(b"Pool Icon URL"),
        };
        pool
    }

    
    public fun get_token_amounts<A,B>(pool: &Pool<A,B>, position: &Position) : (u64, u64) {
        let token_a_amount = (balance::value(&pool.coin_a));
        let token_b_amount = (balance::value(&pool.coin_b));
